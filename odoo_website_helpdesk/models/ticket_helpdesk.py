...
class TicketHelpDesk(models.Model):
    ...
    def _compute_product_display(self):
        for record in self:
            if len(record.product_ids) == 1:
                record.product_display = record.product_ids.name_get()[0][1]
            elif len(record.product_ids) > 1:
                record.product_display = f"{len(record.product_ids)} Products"
            else:
                record.product_display = "No Products"
    ...
    # pentru afisare, date calculate pentru format:
    ...
    def _compute_start_date_only(self):
        for record in self:
            record.start_date_only = record.start_date.date() if record.start_date else False
    ...
    def _compute_end_date_only(self):
        for record in self:
            record.end_date_only = record.end_date.date() if record.end_date else False
    ...
    #campuri noi: punct de lucru si contact
    work_point_id = fields.Many2one(
        'res.partner',
        string='Work Point',
        domain="[('parent_id', '=', customer_id), ('type', '=', 'other')]",
    ...
    # selectare date despre noi clientul curent
    @api.onchange('customer_id')
    def _onchange_customer_id(self):
        """Filter work points and contact persons based on selected customer."""
        if self.customer_id:
            return {
                'domain': {
                    'work_point_id': [('parent_id', '=', self.customer_id.id)],
                    'contact_person_id': [('parent_id', '=', self.customer_id.id)],
                }
            }
        return {'domain': {'work_point_id': [], 'contact_person_id': []}}
    ...
    def action_create_tasks(self):
        """Task creation"""
        # Generăm un timestamp unic
        timestamp = datetime.now().strftime('%Y-%m-%d-%H-%M')
        task_id = self.env['project.task'].create({
            'name': f"{self.name}-{self.subject}-{timestamp}",  # Adăugăm timestamp-ul
            'project_id': self.project_id.id,
